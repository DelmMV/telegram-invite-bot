...
async function checkBotPermissions() {
	try {
		...
		if (
			...
			logger.error(
				'Bot is not an administrator in the group. Some features may not work correctly.'
			)
			console.warn(
				'⚠️ Бот не является администратором группы. Некоторые функции могут не работать корректно.'
			)
		} else {
...
bot.action('apply_for_membership', async ctx => {
	try {
		...
		if (hasPending) {
			...
			return ctx.reply(
				'У вас уже есть активная заявка на вступление в группу. Пожалуйста, дождитесь её рассмотрения администраторами.'
			)
		}
...
bot.action(/^(approve|reject)_(\d+)$/, async ctx => {
	try {
		...
		const application = await applicationService.getApplicationByUserId(
			applicantId
		)
				...
				`${ctx.callbackQuery.message.text}\n\n` +
					...
					`<b>Обработал:</b> ${
						application.processedBy
							? `Администратор ID ${application.processedBy}`
							: 'Неизвестно'
					}\n` +
					`<b>Дата:</b> ${
		...
		if (action === 'approve') {
			...
			if (isMember) {
				...
				await ctx.editMessageText(
					`${ctx.callbackQuery.message.text}\n\n` +
						...
						`<b>Обработал:</b> ${ctx.from.first_name} ${
							ctx.from.last_name || ''
						}\n` +
			...
			try {
				...
				await ctx.answerCbQuery(
					'Заявка одобрена. Пользователь получил ссылку для вступления.'
				)
				await ctx.editMessageText(
					`${ctx.callbackQuery.message.text}\n\n` +
						`<b>Решение:</b> Одобрено ✅\n` +
				...
				if (error.description) {
					if (error.description.includes('not enough rights')) {
						errorMessage +=
							' Бот не имеет необходимых прав администратора в группе.'
					} else if (error.description.includes('chat not found')) {
						errorMessage +=
							' Группа не найдена. Проверьте ID группы в настройках.'
					} else {
				...
				await ctx.reply(
					errorMessage + ' Пожалуйста, проверьте настройки бота и группы.'
				)
			}
		} else {
			// Отклонение заявки
		...
		try {
			...
			logger.error(
				`Failed to edit message with error details: ${editError.message}`
			)
			await ctx.reply(
				'Произошла ошибка при обработке вашего решения. Пожалуйста, попробуйте снова позже.'
			)
		}
	}
})
...
bot.catch((err, ctx) => {
	...
	ctx
		.reply('Произошла непредвиденная ошибка. Пожалуйста, попробуйте позже.')
		.catch(e => {})
})
